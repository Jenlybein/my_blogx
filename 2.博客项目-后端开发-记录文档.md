...
cp /etc/letsencrypt/live/你的域名/fullchain.pem \
   /opt/myblogx/myblogx_server/deploy/nginx/cert/domain.pem

cp /etc/letsencrypt/live/你的域名/privkey.pem \
   /opt/myblogx/myblogx_server/deploy/nginx/cert/domain.key

chmod 600 /opt/myblogx/myblogx_server/deploy/nginx/cert/domain.key
chmod 644 /opt/myblogx/myblogx_server/deploy/nginx/cert/domain.pem
```
...
# 六、非功能性需求（忽略此部分，暂不更改，后续视开发进度进行补充）
