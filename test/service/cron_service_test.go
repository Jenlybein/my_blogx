...
func TestSyncArticleApplyIncrements(t *testing.T) {
	...
	if err := db.Model(&models.ArticleModel{}).
		Select("view_count", "digg_count", "favor_count").
		Where("id = ?", article.ID).
		Scan(&got).Error; err != nil {
		t.Fatalf("查询文章失败: %v", err)
	}
	if got.ViewCount != 5 || got.DiggCount != 2 || got.FavorCount != 3 {
		t.Fatalf("同步结果异常: view=%d digg=%d favor=%d", got.ViewCount, got.DiggCount, got.FavorCount)
	}
}
...
func TestSyncArticleNotBelowZeroAndLockSkip(t *testing.T) {
	...
	if got.DiggCount != 0 || got.FavorCount != 0 {
		t.Fatalf("计数不应小于0: digg=%d favor=%d", got.DiggCount, got.FavorCount)
	}
