...
- 当前图片系统已经重构为“上传任务 + 对象存储直传 + 服务端验收入库”模式，不再走旧的后端 `multipart/form-data` 接收上传。
- 上传入口要求登录，当前围绕七牛云对象存储工作。
- 图片上传分成三段：
  - 创建上传任务
  - 客户端直传对象存储
  - 后端确认上传结果并生成正式图片记录
- 系统支持基于图片内容哈希的去重：
  - 如果数据库里已存在同内容图片，可直接秒传返回，不再重复上传
  - 如果上传后才发现和已有图片重复，也会复用旧图片记录并清理重复对象
- 后台已支持图片列表查询和批量删除。

#### 实现思路

当前图片链路的核心不是“上传文件”，而是“先创建任务，再验收文件”。这样做的目标，是把上传过程拆成“声明、传输、验收”三个阶段，既减少后端带宽，又把安全校验放回服务端收口。
...
1. **创建上传任务**
   - 前端先提交图片元信息：
     ...
     - `mime_type`
     - `hash`
   - 后端在 `CreateUploadTask` 中先做第一轮静态校验：
     - 七牛配置是否完整
     - 大小是否超过配置上限
     - MIME 是否是 `image/*`
     - 文件后缀是否在白名单内
     - 内容哈希是否为空
   - 如果数据库里已经存在相同 `hash` 的正式图片，接口会直接返回 `skip_upload=true` 和现有图片信息，形成“秒传”体验。
   - 如果没有命中去重，则生成一条上传任务：
     - 任务 ID 使用雪花 ID
     - 存储 Key 按 `前缀/images/日期/hash` 组织
     - 任务状态初始为 `pending`
     - 任务本身不先进数据库，而是暂存到 Redis

2. **签发对象存储上传凭证**
   - 后端给每个任务签发一份七牛上传凭证，而不是自己接收文件内容。
   - 上传凭证会约束：
     - 固定 bucket
     - 固定 object key
     - 最大文件大小
     - 仅允许图片 MIME
     - `InsertOnly=1`，避免覆盖写
   - 如果配置了回调地址，七牛上传成功后会自动回调后端；未配置回调时，也保留前端手动确认接口作为兜底。

3. **上传任务缓存与状态管理**
   - 上传任务存储在 Redis，而不是数据库。
   - Redis 中同时维护两类索引：
     - `task_id -> 任务数据`
     - `object_key -> task_id`
   - 这样做有两个目的：
     - 前端可以按任务 ID 轮询状态
     - 七牛回调可以按 object key 反查任务
   - 任务在待上传阶段会跟随上传凭证过期时间一起失效；已完成或失败的任务会继续保留一段时间，方便前端查询最终状态。

4. **服务端确认上传**
   - 上传完成后的确认有两条入口，但最终都会收敛到同一套核心逻辑：
     - 七牛回调：`POST /api/images/qiniu/callback`
     - 手动兜底：`POST /api/images/upload-tasks/complete`
   - 确认时不会盲信前端或对象存储返回值，而是再次向七牛查询真实对象信息，完成第二轮验收：
     - 对象是否存在
     - 哈希是否与任务一致
     - 文件大小是否与声明一致
     - MIME 是否仍是图片
     - 图片格式是否在白名单中
     - 宽高是否合理
   - 也就是说，真正决定“这是不是一张可用图片”的，不是客户端声明，而是后端对对象存储中真实文件的二次校验。

5. **并发控制与正式入库**
   - 为了避免重复确认同一个上传任务，确认阶段会先对任务加 Redis 分布式锁。
   - 验收通过后，再进入数据库事务：
     - 如果按哈希查到已有图片，则直接复用旧记录
     - 如果没有，就创建新的 `image_models`
     - 然后把任务状态改成 `ready`，挂上 `image_id`
   - 如果发现这次上传其实和已有图片重复，但对象 key 不同，系统会在事务完成后尝试删除重复上传到七牛的对象，避免对象存储里留下多份冗余文件。

6. **图片管理**
   - 后台图片列表直接查 `image_models`，按文件名和对象 key 支持模糊搜索。
   - 批量删除时，先删除七牛对象，再物理删除数据库记录，避免出现“数据库没了但云上对象还在”的残留资源。
